...
def outputResults(output,output_file=None,output_type=None,volume_information=None):
    if output_type:
		...
		json_output2 = {}
		for key, value in json_output['Run Times'].items():
			print value
			json_output_update = {
				key : {'Executable Name' : json_output['Executable Name'],
				'Prefetch Hash': json_output['Prefetch Hash'],
				key : value}
			}
			if value != "N/A":	
				json_output2.update(json_output_update)

		
		print json.dumps(json_output2,indent=4,sort_keys=True)
